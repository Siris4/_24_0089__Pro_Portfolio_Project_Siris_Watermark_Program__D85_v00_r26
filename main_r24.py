    ...
    if grid_mode.get():
        ...
        for x in range(0, base_image.width, spacing_x):
            for y in range(0, base_image.height, spacing_y):
                # Apply watermark only on even (x + y) sum positions to create checkered pattern
                if (x + y) % (2 * spacing_x) == 0:
...
def update_text_watermark():
    ...
    if grid_mode.get():
        ...
        for x in range(0, base_image.width, spacing_x):
            for y in range(0, base_image.height, spacing_y):
                # Apply text only on odd (x + y) sum positions to create checkered pattern
                if (x + y) % (2 * spacing_x) != 0:
